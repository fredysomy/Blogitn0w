...
router.route("/register").post((req, res) => {
  bcy.hash(req.body.pass, 10, function (err, hash) {
    ...
    (somem.realname = req.body.realname), (somem.name = req.body.name);
    ...
    somem.save((err, doc) => {
      if (!err) {
        ...
        res.render("index", {
          err: '<p style="color:red;">User exists <br> Change username</p>',
        });
      }
    });
  });
...
router.route("/verify/:id").get((req, res) => {
  tksc.findOne({ token: req.params.id }, (err, doc) => {
    usersch
      .update({ _id: doc._userId }, { $set: { verify: true } })
      .then(res.redirect("/user/login"));
  });
});
router.route("/signin").post((req, res) => {
  usersch
    .findOne({ name: req.body.name, email: req.body.email })
    .then((user) => {
      if (!user) {
        res.render("login", {
          err:
            '<h6 style="color:red;"></h6>User does not exist</h6><br><a href="/user/login">Try again</a><br>',
        });
      } else {
        bcy.compare(req.body.pass, user.pass, (err, result) => {
          if (result == true) {
            if (user.verify == true) {
              ...
              res.render("login", {
                err:
                  '<h6 style="color:red;"></h6>User not verified</h6><br><a href="/user/login">Try again</a><br>',
              });
            }
          }
          if (err) {
            res.render("login", {
              err:
                '<h6 style="color:red;"></h6>Username or password does not match</h6><br><a href="/user/login">Try again</a><br>',
            });
          }
          if (result == false) {
            res.render("login", {
              err:
                '<h6 style="color:red;"></h6>Wrong credentials<br>Please register-</h6><br><a href="/user/login">Try again</a><br>',
            });
          }
        });
      }
...
router.route("/u").get((req, res) => {
  if (req.session.user) {
    blogsc
      .find({ mailuser: req.session.user.email })
      .sort({ daten: -1 })
      .then((data3) => {
...
router.route("/updet").get((req, res) => {
  if (req.session.user) {
    usersch
      .findOne({ email: req.session.user.email, name: req.session.user.name })
      .then((data) => {
        res.render("upcred", {
          desc: data.descr,
        });
...
router.route("/add/blg").post((req, res) => {
  ...
  blgs.name = req.session.user.realname;
...
router.route("/blog/:id").get((req, res) => {
  if (req.session.user) {
    blogsc
      .findOne({ _id: req.params.id, mailuser: req.session.user.email })
      .then((data) => {
...
router.route("/edit/blg").post((req, res) => {
  blogsc
    .updateOne(
      { _id: req.body.id, mailuser: req.session.user.email },
      { $set: { head: req.body.head, blog: req.body.desc } }
    )
    .then(res.redirect("/user/u"));
});
...
router.route("/update/cred").post((req, res) => {
  if (req.body.pass == req.body.pass2) {
    bcy.hash(req.body.pass2, 10, (err, doc) => {
      usersch
        .updateOne({ email: req.session.user.email }, { $set: { pass: doc } })
        .then(res.redirect("/user/u"));
      console.log(doc);
    });
  } else {
